...
import { getHrefPost, getMarkdownPost } from './post.ts'
...
export const getOrderedPosts: () => Promise<Map<string, Ordered<Post>>> = cache(
  async () => {
    const [markdownEntries, hrefEntries] = await Promise.all([
      getCollection(`markdownPosts`),
      getCollection(`hrefPosts`),
    ])
    const posts: Ordered<Post>[] = [
      ...(await Promise.all(markdownEntries.map(getMarkdownPost))),
      ...hrefEntries.map(getHrefPost),
    ].sort(
      (post1, post2) =>
