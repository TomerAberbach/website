...
import type { Dates } from '~/lib/format.ts'
import {
  formatDatesForDisplay,
  formatMinutesToRead,
} from '~/lib/format.ts'
import { SITE_TITLE_AND_AUTHOR } from '~/lib/meta.ts'

// Read the source asset bytes directly (sharp needs the raw file, not a hashed
// URL). `process.cwd()` is the project root during the build.
const mediaPath = (name: string): string =>
  join(process.cwd(), `src/assets/media`, name)
...
const getLogo = cache(async () =>
  renderPngWithInfo(
    sharp(await fs.readFile(mediaPath(`logo.svg`))).resize({
      width: LOGO_WIDTH,
    }),
  ),
)
    ...
    .composite([
      {
        ...
        input: await renderPng(
          sharp(await fs.readFile(mediaPath(`avatar.png`)))
